"""Compute uplift@10% and uplift@30% for sections 8, 11, 12 of modeling.ipynb."""
...
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*80)
print("SECTION 8: Balance experiments")
print("="*80)

# Build balanced datasets (same as notebook)
n_ctrl = int((treatment_train == 0).sum())
n_trt  = int((treatment_train == 1).sum())

# Downsample treatment to match control
rng = np.random.RandomState(42)
trt_idx  = np.where(treatment_train == 1)[0]
ctrl_idx = np.where(treatment_train == 0)[0]
trt_down = rng.choice(trt_idx, size=n_ctrl, replace=False)
down_idx = np.concatenate([trt_down, ctrl_idx])
X_down = X_train.iloc[down_idx].reset_index(drop=True)
y_down = y_train.iloc[down_idx].reset_index(drop=True)
t_down = treatment_train.iloc[down_idx].reset_index(drop=True)

# Oversample control to match treatment
ctrl_over = rng.choice(ctrl_idx, size=n_trt, replace=True)
over_idx = np.concatenate([trt_idx, ctrl_over])
X_over = X_train.iloc[over_idx].reset_index(drop=True)
y_over = y_train.iloc[over_idx].reset_index(drop=True)
t_over = treatment_train.iloc[over_idx].reset_index(drop=True)

# IPW weights
p = treatment_train.mean()
ipw_weights = np.where(treatment_train == 1, 1/p, 1/(1-p))

bal_results = []

configs_bal = [
    ('Class Transformation', '75/25 (as is)',    X_train, y_train, treatment_train, None),
    ('Class Transformation', '50/50 (downsample)', X_down, y_down, t_down,  None),
    ('Class Transformation', '50/50 (oversample)', X_over, y_over, t_over,  None),
    ('Class Transformation', 'IPW weighting',    X_train, y_train, treatment_train, ipw_weights),
]

for model_name, balance, X_tr, y_tr, t_tr, weights in configs_bal:
    t0 = time.time()
    m = ClassTransformation(lgbm())
    if weights is not None:
        m.fit(X_tr, y_tr, t_tr, estimator_fit_params={'sample_weight': weights})
    else:
        m.fit(X_tr, y_tr, t_tr)
    pred = m.predict(X_test)
    row = {'Model': model_name, 'Balance': balance, **metrics(y_test, pred, treatment_test)}
    bal_results.append(row)
    print(f"  {model_name} {balance}: Qini={row['Qini AUC']:.4f}  u@10%={row['uplift@10%']:.4f}  u@30%={row['uplift@30%']:.4f}  ({time.time()-t0:.1f}s)")

for model_name, Model in [('T-Learner', lambda: TwoModels(lgbm(), lgbm(), method='vanilla')),
                            ('S-Learner', lambda: SoloModel(lgbm(), method='dummy'))]:
    for balance, X_tr, y_tr, t_tr in [
        ('75/25 (as is)',    X_train, y_train, treatment_train),
        ('50/50 (downsample)', X_down, y_down, t_down),
        ('50/50 (oversample)', X_over, y_over, t_over),
    ]:
        t0 = time.time()
        m = Model()
        m.fit(X_tr, y_tr, t_tr)
...
from causalml.inference.meta import BaseXRegressor, BaseDRLearner, BaseRRegressor
...
t_tr_arr = treatment_train.values

# Propensity scores via cross_val_predict
print("  Computing propensity scores...")
from sklearn.linear_model import LogisticRegression
prop_clf = LGBMClassifier(n_estimators=100, max_depth=4, random_state=42, verbose=-1)
p_train = cross_val_predict(prop_clf, X_tr_arr, t_tr_arr, cv=5, method='predict_proba')[:, 1]
prop_clf.fit(X_tr_arr, t_tr_arr)
p_test  = prop_clf.predict_proba(X_te)[:, 1]
...
print("  X-Learner...")
t0 = time.time()
xl = BaseXRegressor(learner=LGBMClassifier(n_estimators=200, max_depth=5, random_state=42, verbose=-1))
xl.fit(X=X_tr_arr, treatment=t_tr_arr, y=y_tr_arr, p=p_train)
...
print("  DR-Learner...")
t0 = time.time()
dr = BaseDRLearner(learner=LGBMClassifier(n_estimators=200, max_depth=5, random_state=42, verbose=-1))
dr.fit(X=X_tr_arr, treatment=t_tr_arr, y=y_tr_arr, p=p_train)
...
print("  R-Learner...")
t0 = time.time()
rl = BaseRRegressor(learner=LGBMClassifier(n_estimators=200, max_depth=5, random_state=42, verbose=-1))
rl.fit(X=X_tr_arr, treatment=t_tr_arr, y=y_tr_arr, p=p_train)
...
sub_idx = rng2.choice(len(X_tr_arr), size=80000, replace=False)
X_sub = X_tr_arr[sub_idx]; y_sub = y_tr_arr[sub_idx]; t_sub = t_tr_arr[sub_idx]
cf = CausalForestDML(n_estimators=100, max_depth=5, random_state=42, n_jobs=-1)
...
smote_results = []
for label, X_tr, y_tr, t_tr in smote_configs:
    t0 = time.time()
    if label.startswith('S'):
        m = SoloModel(lgbm(), method='dummy')
    else:
        m = TwoModels(lgbm(), lgbm(), method='vanilla')
    m.fit(X_tr, y_tr, t_tr)
    pred = m.predict(X_test)
