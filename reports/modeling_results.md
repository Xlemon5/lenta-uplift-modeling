...
Дополнительно протестированы (Секция 12): X-Learner, DR-Learner, R-Learner (`causalml` 0.16.0) и CausalForestDML (`econml` 0.16.0). Все значительно уступают CT baseline по Qini AUC (−87–94%). Также проверена гипотеза CT-guided SMOTE на топ-10% клиентов (Секция 13): гипотеза опровергнута, результат −59–73%.
...
- **Калибровка предсказаний CT** — для получения интерпретируемых абсолютных значений uplift
- **Стекинг моделей** — ансамбль из CatBoost и LightGBM ClassTransformation
- **Uplift-деревья (CausalTree, UpliftRandomForest)** — альтернативный подход к прямой оптимизации uplift
...
## 13. CT-guided SMOTE на топ-10% клиентов
...
CT-модель выявила, что топ-10% клиентов по предсказанному uplift демонстрируют фактический uplift ≈ +17.9 п.п. (против среднего +0.75 п.п. по всей выборке). Концентрация сигнала в этой зоне в 24 раза выше среднего.

**Гипотеза:** S-Learner и T-Learner слабы на полном датасете (+480K строк) потому, что сигнал «разбавлен» нечувствительными клиентами. Если обучать их только на топ-10% — там где uplift-сигнал силён — они смогут лучше уловить паттерн и перенести ранжирование на полный тест.

Дополнительно: внутри топ-10% T/C соотношение уже ближе к балансу (58%/42%) — применяем SMOTE до 50/50 для выравнивания обучения T-Learner.

### 13.2 Параметры эксперимента

- **Обучающая выборка:** топ-10% по CT-скору на train (48,092 строк из 480,920)
- **T/C ratio в топ-10%:** 58.1% / 41.9% (vs 75.1%/24.9% в полном train)
- **CR в топ-10%:** 37.29% (vs 10.6% в полном train) — высокая концентрация отзывчивых клиентов
- **SMOTE:** генерация 7,796 синтетических control-наблюдений → balanced subset 55,888 строк, T rate = 50.0%
...
| Модель | Qini AUC | Uplift AUC | ASD | Время |
|--------|----------|------------|-----|-------|
| S-Learner (full 75/25) | 0.0110 | 0.0061 | 0.000038 | 15.9s |
| **S-Learner (top-10% SMOTE)** | **0.0045** | **0.0020** | 0.205307 | 6.5s |
| T-Learner (full 75/25) | 0.0140 | 0.0077 | 0.000915 | 18.6s |
| **T-Learner (top-10% SMOTE)** | **0.0038** | **0.0016** | 0.373731 | 8.9s |

**Delta vs baseline:**
- S-Learner: 0.0110 → 0.0045 (**−58.8%**)
- T-Learner: 0.0140 → 0.0038 (**−72.8%**)

### 13.4 Анализ: почему гипотеза не подтвердилась

**Причина 1 — Train/Test domain mismatch (главная).**
Модели обучаются на 55K строк из топ-10%, но тест — это 206K строк всей выборки. Вне топ-10% (90% теста) модели видели слишком мало паттернов при обучении и плохо обобщаются. Это классический случай «out-of-distribution generalization failure».

**Причина 2 — Потеря информации.**
Полный train (480K) содержит разнообразные паттерны нечувствительных клиентов — именно они помогают модели понять, «кто НЕ откликается». Убирая 90% данных, мы лишаем модель информации об этой противоположной группе.

**Причина 3 — ASD растёт катастрофически.**
ASD S-Learner вырос с 0.000038 до 0.205307 (×5400!), T-Learner с 0.000915 до 0.373731 (×408). Это означает: предсказанный uplift в децилях полностью расходится с фактическим — модели «думают», что все клиенты похожи на топ-10%, и предсказывают завышенный uplift для нечувствительных клиентов.

**Причина 4 — Аналогия с Section 11.**
SMOTE на полном control (Section 11) дало −41% у T-Learner. CT-guided SMOTE на топ-10% даёт −72.8% — ещё хуже, т.к. к проблеме синтетических данных добавляется ещё и проблема out-of-distribution.

### 13.5 Вывод

Гипотеза опровергнута. Обучение S-Learner / T-Learner на «концентрированном» топ-10% подмножестве с SMOTE-балансировкой **значительно ухудшает** Qini AUC:
- S-Learner: −58.8%
- T-Learner: −72.8%

Для моделей, оценивающих conditional outcomes (S-Learner, T-Learner), критически важно охватить всё пространство признаков при обучении. Выборочное обучение на «лучших» клиентах создаёт смещение и плохое обобщение.

**Вывод подтверждает итог всей серии балансировочных экспериментов (Секции 8–12):** любые попытки улучшить S-Learner и T-Learner через балансировку или фильтрацию данных не помогают. Оптимальная стратегия остаётся неизменной — **Class Transformation + LightGBM на оригинальных данных (Qini AUC = 0.0752)**.

---

## 14. Интерпретация метрик и оценка качества результатов

### 13.1 Что означают значения ASD
...
|--------|----------|-----|-----|
| **CT + LightGBM** | **0.0752** | 0.184 | Лучший ранжировщик, некалиброванная шкала |
| S-Learner (dummy) | 0.0100 | **0.000018** | Лучшая калибровка (предсказывает ΔP) |
...
|--------|---------|---------------|
| Таргетинг: кому отправить рассылку | Qini AUC | **CT + LightGBM** |
| Оценка ROI: на сколько % вырастет конверсия | ASD / калибровка | **DR/R-Learner** |

---

### 13.2 Насколько хороши наши результаты?
...
|----------|--------|-------------|
| Лучше случайного выбора | ✅ | Qini AUC в 75× выше random |
| Концентрация отзывчивых клиентов | ✅ Отличная | uplift@10% = 18 п.п. = 24× ATE |
| Переобучение (Train/Test gap) | ✅ Умеренное | 1.37× — приемлемо |
