...
### 16.4 Визуализация: uplift по децилям

Графики показывают фактический uplift (CR_T − CR_C) в каждом из 10 децилей на тестовой выборке. Клиенты отсортированы по **предсказанному** uplift от модели.

**CT + LightGBM (baseline):**

![Uplift по децилям — CT baseline](figures/decile_ct_baseline.png)

**CT + Linear OLS:**

![Uplift по децилям — CT + Linear OLS](figures/decile_ct_linear_ols.png)

**CT + Isotonic (OOF+IPW):**

![Uplift по децилям — CT + Isotonic](figures/decile_ct_isotonic.png)

#### Ключевые наблюдения

| Дециль | CT baseline / CT+Linear | CT + Isotonic |
|--------|------------------------|---------------|
| 10 (топ) | **0.156** | 0.081 |
| 9 | 0.031 | 0.078 |
| 8 | 0.013 | 0.007 |
| 7 | 0.008 | 0.012 |
| 1–6 | 0.001–0.005 | 0.001–0.008 |

**CT baseline и CT + Linear OLS имеют идентичные децили** — линейная калибровка строго монотонна (slope > 0), поэтому ни один клиент не меняет дециль. Ранжирование сохранено точно.

**CT + Isotonic нарушает монотонность:** дециль 9 (0.078) почти сравнялся с децилем 10 (0.081) — изотоническая регрессия создаёт плато, в которые «сливается» несколько исходных CT-скоров. Клиенты, которые в CT baseline попадали в дециль 10, теперь распределяются между децилями 9 и 10 случайно. Это прямая причина падения uplift@10% с **0.179 до 0.080** (−55%).
...
**Метод 1 (Isotonic):** ASD −99.5% (лучше Linear OLS), но Qini −8.5%, uplift@10% −55%. Изотоническая регрессия создаёт плато в пространстве скоров — многие CT-баллы отображаются в одно CATE-значение, разрушая точное ранжирование в топ-дециле. Для задачи таргетинга uplift@10% критичен, поэтому Isotonic хуже Linear OLS.
...
Включены все ключевые модели, протестированные в проекте. Для train — in-sample предсказания (показывает переобучение). Исключение: **CT + Linear (OLS)** и **CT + Isotonic** используют OOF-предсказания на трейне (честная оценка без переобучения). Train ASD для CT + Linear оценочный (~0.002), точное значение не вычислялось.
...
| CT + Linear (OLS) *(OOF)* | 0.0856 | 0.0352 | 0.0677 | 0.0397 | 0.0186 | ~0.002 |
...
| CT + Linear (OLS) *(OOF)* | 0.0677 | 0.0752 | **1.0×** | ✅ Нет переобучения (OOF) |
