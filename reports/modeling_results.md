...
- **Шаблон SMOTE:** control-наблюдения из топ-10% CT-скора: **20,148 строк**, CR = 0.1774
- **Сравнение (Секция 11):** шаблон — весь control-пул: 119,796 строк, CR = 0.1026
- **Генерация:** 241,328 синтетических control-строк (чтобы привести датасет к 50/50 T/C)
- **Обучающая выборка:** 480,920 (реальных) + 241,328 (синтетических) = **722,248 строк**, T rate = 50.0%
...
|--------|----------|------------|-----|-------|
| S-Learner (original 75/25) | 0.0110 | 0.0061 | 0.000038 | 15.6s |
| S-Learner (SMOTE full ctrl, Секция 11) | 0.0109 | — | — | — |
| S-Learner (SMOTE top-10% ctrl) | 0.0035 | 0.0018 | 0.000593 | 29.7s |
| T-Learner (original 75/25) | 0.0140 | 0.0077 | 0.000915 | 23.1s |
| T-Learner (SMOTE full ctrl, Секция 11) | 0.0083 | — | — | — |
| **T-Learner (SMOTE top-10% ctrl)** | **0.0189** | **0.0103** | 0.001683 | 37.2s |

**Delta vs original:**
- S-Learner: 0.0110 → 0.0035 (**−68.2%**)
- T-Learner: 0.0140 → 0.0189 (**+34.7%**)

### 13.4 Анализ

**T-Learner: +34.7% — гипотеза частично подтвердилась.**
...
Итог T-Learner (+34.7%) сопоставим с простым дублированием control (Секция 8.3: +33.1%), но достигается через более информативный синтез.

**S-Learner: −68.2% — шаблон не помогает.**

S-Learner обучает одну модель на всём датасете с treatment как признаком. Синтетические строки с завышенным CR (≈17.7% vs среднее 10.3%) смещают распределение целевой переменной в control-части датасета, что нарушает баланс и ухудшает обобщение.

**Важное ограничение:** T-Learner с SMOTE top-10% ctrl (Qini AUC = 0.0189) всё ещё в **4× хуже** лучшей модели (CT + LGB: 0.0752).

### 13.5 Вывод

CT-guided SMOTE как шаблон для генерации **помогает T-Learner (+34.7%)**, но абсолютное значение остаётся далеко от лидера. S-Learner ухудшается (−68.2%).

Синтетические control-клиенты с высокоаплифтным профилем улучшают оценку counterfactual E[Y(0)|X] для «отзывчивых» клиентов — но этого недостаточно, чтобы преодолеть фундаментальное преимущество Class Transformation, который прямо оптимизирует uplift-ранжирование.
...
### 14.1 Что означают значения ASD
...
### 14.2 Насколько хороши наши результаты?
